...
object Interaction2 {
  ...
  def yearSelection(selectedLayer: Signal[Layer], sliderValue: Signal[Int]): Signal[Int] = {
    Signal(math.max(math.min(sliderValue(), selectedLayer().bounds.max), selectedLayer().bounds.min))
  }
