...
class BlurSuite extends FunSuite {
  def printBox(box : Img): Unit = {
    for(i <- 0 until box.width; j <- 0 until box.height) {
      print("[" + red(box.apply(i,j)) + " "  + green(box.apply(i,j)) + " "  + blue(box.apply(i,j)) + " "  + alpha(box.apply(i,j)) + "]")
      if(j == box.height - 1) println
    }
  }
  ...
  test("boxBlurKernel should correctly handle radius 0") {
    ...
    for (x <- 0 until 5; y <- 0 until 5)
      assert(boxBlurKernel(src, x, y, 0) === rgba(x, y, x + y, math.abs(x - y)),
        "boxBlurKernel(_,_,0) should be identity." + x + ":" + y)
  }
