...
class KMeansSuite extends FunSuite {
  ...
  test("'kMeans' should work for 'points' == GenSeq((0, 0, 1), (0,0, -1), (0,1,0), (0,10,0)) and 'oldMeans' == GenSeq((0, -1, 0), (0, 2, 0)) and 'eta' == 12.25") {
    ...
    val expected : GenSeq[Point] = IndexedSeq(new Point(0, 0, 0), new Point(0.0, 5.5, 0.0))
    assert(kMeans(points, means, 12.25) == expected,
      s"KMeans($points par, $means par) should equal to $expected")
  }
}
