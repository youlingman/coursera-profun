...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType =
    columnNames.tail.foldLeft((new StructType).add(columnNames.head, "string", false)) { (s, name) => s.add(name, "double", false) }
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column = when($"telfs" >= 1.0 and $"telfs" < 3.0, "working").otherwise("not working").as("working")
