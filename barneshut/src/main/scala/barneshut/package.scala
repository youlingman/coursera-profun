...
package object barneshut {
  ...
  case class Fork(
    ...
    val size: Float = Math.max(nw.size + ne.size, sw.size + se.size)
    ...
    def insert(b: Body): Quad =
      if (size <= minimumSize) Leaf(centerX, centerY, size, b +: bodies)
        ...
        var fork = Fork(Empty(centerX - size / 4, centerY - size / 4, size / 2),
          ...
          Empty(centerX - size / 4, centerY + size / 4, size / 2),
          Empty(centerX + size / 4, centerY + size / 4, size / 2))
        for (b <- bodies) fork = fork.insert(b)
        fork
      }
  }
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val x = (b.x - boundaries.minX) / (boundaries.width / sectorPrecision)
      val y = (b.y - boundaries.minY) / (boundaries.height / sectorPrecision)
      this.apply(Math.min(x.toInt,sectorPrecision-1), Math.min(y.toInt,sectorPrecision-1)) += b
      this
    }
