...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    val mergeBd = new Boundaries
    mergeBd.minX = Math.min(a.minX, b.minX)
    mergeBd.maxX = Math.max(a.maxX, b.maxX)
    mergeBd.minY = Math.min(a.minY, b.minY)
    mergeBd.maxY = Math.max(a.maxY, b.maxY)
    mergeBd
  }
